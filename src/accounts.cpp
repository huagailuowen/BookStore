...
#include <vector>
// #include "mytype.h"
bool Account::operator<(const Account &b) const { return id < b.id; }
bool Account::operator>(const Account &b) const { return id > b.id; }
bool Account::operator==(const Account &b) const { return id == b.id; }
bool Account::operator<=(const Account &b) const { return id <= b.id; }
bool Account::operator>=(const Account &b) const { return id >= b.id; }
...
power_type Account::privilege() { return power; }
Password Account::pass() { return password; }
Userid Account::user() { return id; }
Username Account::nam() { return name; }
...
void AccountData::del(Userid id) { Accountware.del(id); }
...
void AccountData::adduser(Account the) { Accountware.insert(the.id, the); }