...
#include<set>
#include <utility>
bool Log::operator<(const Log &b)const
{
    return id<b.id;
}
bool Log::operator>(const Log &b)const
...
std::pair<double,double> LogData::financesum(int count)
{
    ...
    std::vector<std::pair<int,std::pair<double,double>>>res=sumw.findsegment(base.second.first-count+1, base.second.first);
...
void LogData::addcrew(Userid id)
{
    crew.insert(1,id);
}
void LogData::delcrew(Userid id)
{
    crew.del(1,id);
}
